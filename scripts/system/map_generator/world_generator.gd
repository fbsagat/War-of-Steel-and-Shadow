...
# --- Configurações de Relevo Personalizáveis ---
@export_category("Configurações de Relevo Personalizáveis")
@export_enum("Flat", "Semi-Flat", "Gentle Hills", "Rolling Hills", "Valleys", "Custom") 
var tipo_relevo_padrao: String = "Semi-Flat"

@export_subgroup("Flat")
@export var flat_amplitude: float = 0.0
@export var flat_frequencia: float = 0.01
@export var flat_octaves: int = 1
@export var flat_persistencia: float = 0.5
@export var flat_lacunaridade: float = 2.0

@export_subgroup("Semi-Flat")
@export var semi_flat_amplitude: float = 1.2
@export var semi_flat_frequencia: float = 0.012
@export var semi_flat_octaves: int = 2
@export var semi_flat_persistencia: float = 0.35
@export var semi_flat_lacunaridade: float = 2.0

@export_subgroup("Gentle Hills")
@export var gentle_hills_amplitude: float = 4.0
@export var gentle_hills_frequencia: float = 0.022
@export var gentle_hills_octaves: int = 2
@export var gentle_hills_persistencia: float = 0.35
@export var gentle_hills_lacunaridade: float = 2.0

@export_subgroup("Rolling Hills")
@export var rolling_hills_amplitude: float = 7.0
@export var rolling_hills_frequencia: float = 0.028
@export var rolling_hills_octaves: int = 3
@export var rolling_hills_persistencia: float = 0.4
@export var rolling_hills_lacunaridade: float = 2.0

@export_subgroup("Valleys")
@export var valleys_amplitude: float = 14.0
@export var valleys_frequencia: float = 0.015
@export var valleys_octaves: int = 2
@export var valleys_persistencia: float = 0.25
@export var valleys_lacunaridade: float = 2.0

@export_subgroup("Custom")
@export var custom_amplitude: float = 5.0
@export var custom_frequencia: float = 0.05
@export var custom_octaves: int = 4
@export var custom_persistencia: float = 0.5
@export var custom_lacunaridade: float = 2.0
...
@export var tamanho_zona_transicao: float = 0.4 ## Tamanho da zona de blend (0-1)
...
@export var subdivisions: int = 24 ## Subdivisões do mesh (qualidade)
...
func _configurar_noise() -> void:
	...
	# Não definimos frequency, octaves, etc. aqui,
...
func _obter_altura_no_ponto(world_x: float, world_z: float) -> float:
	...
	# Se NÃO usa etapas ou está no modo flat, comportamento normal
	if not usar_etapas_relevo or etapas_relevo.is_empty() or raio_mapa_metros <= 0:
		var params: Dictionary = _obter_parametros_base(tipo_relevo_padrao)
		var altura: float
		if params.tipo == "Flat":
			altura = 0.0
		else:
	...
	# === Interpolação baseada em altura, não em parâmetros ===
	var percentual: float = clamp(distancia_centro / raio_mapa_metros, 0.0, 1.0)
	...
	for i in range(etapas_relevo.size()):
		...
		if percentual <= percentual_acumulado + percentual_etapa:
			if suavizar_transicoes and etapa_anterior != null:
				...
				if percentual < inicio_etapa + zona_blend and zona_blend > 0.0:
					var blend: float = (percentual - inicio_etapa) / zona_blend
					
					var params_a: Dictionary = _obter_parametros_base(etapa_anterior.tipo_relevo)
					var params_b: Dictionary = _obter_parametros_base(etapa.tipo_relevo)
					
					var altura_a: float = 0.0 if params_a.tipo == "Flat" else _gerar_altura_com_noise(world_x, world_z, params_a)
					var altura_b: float = 0.0 if params_b.tipo == "Flat" else _gerar_altura_com_noise(world_x, world_z, params_b)
					
					var altura: float = lerp(altura_a, altura_b, blend)
					altura_cache[cache_key] = altura
					return altura
			
			# Fora da zona de blend: usar etapa atual
			var params: Dictionary = _obter_parametros_base(etapa.tipo_relevo)
			var altura: float = 0.0 if params.tipo == "Flat" else _gerar_altura_com_noise(world_x, world_z, params)
			altura_cache[cache_key] = altura
			return altura
	...
	if etapas_relevo.size() > 0:
		var params: Dictionary = _obter_parametros_base(etapas_relevo[-1].tipo_relevo)
		var altura: float = 0.0 if params.tipo == "Flat" else _gerar_altura_com_noise(world_x, world_z, params)
		altura_cache[cache_key] = altura
		return altura
...
func _gerar_altura_com_noise(x: float, z: float, params: Dictionary) -> float:
	...
	var oct_original: int = noise.fractal_octaves
	var gain_original: float = noise.fractal_gain
	var lac_original: float = noise.fractal_lacunarity

	# Aplicar configuração do relevo atual
	noise.frequency = params.frequencia
	noise.fractal_octaves = params.octaves
	noise.fractal_gain = params.persistencia
	noise.fractal_lacunarity = params.lacunaridade

	var x_rounded: float = round(x * 1000.0) / 1000.0
	var z_rounded: float = round(z * 1000.0) / 1000.0

	var raw_noise: float = noise.get_noise_2d(x_rounded, z_rounded)

	# === SUAVIZAÇÃO ADICIONAL PARA EVITAR PONTOS ÁSPEROS ===
	# Aplica um "blur" leve no ruído usando média de vizinhos
	var smooth_factor: float = 0.25  # 0.0 = desligado, 0.3 = suave
	if smooth_factor > 0.0:
		var offset: float = 1.5  # distância dos vizinhos (em metros)
		var n1: float = noise.get_noise_2d(x_rounded + offset, z_rounded)
		var n2: float = noise.get_noise_2d(x_rounded - offset, z_rounded)
		var n3: float = noise.get_noise_2d(x_rounded, z_rounded + offset)
		var n4: float = noise.get_noise_2d(x_rounded, z_rounded - offset)
		raw_noise = lerp(raw_noise, (raw_noise + n1 + n2 + n3 + n4) / 5.0, smooth_factor)

	var altura: float = raw_noise * params.amplitude

	# Restaurar configuração original
	noise.frequency = freq_original
	noise.fractal_octaves = oct_original
	noise.fractal_gain = gain_original
	noise.fractal_lacunarity = lac_original

	return clamp(altura, 0.0, altura_maxima)
	...
	match tipo:
		"Flat":
			return {
				...
				"frequencia": flat_frequencia,
				"octaves": flat_octaves,
				"persistencia": flat_persistencia,
				"lacunaridade": flat_lacunaridade
			}
		"Semi-Flat":
			return {
				"tipo": "Semi-Flat",
				"amplitude": semi_flat_amplitude,
				"frequencia": semi_flat_frequencia,
				"octaves": semi_flat_octaves,
				"persistencia": semi_flat_persistencia,
				"lacunaridade": semi_flat_lacunaridade
			}
		"Gentle Hills":
			return {
				"tipo": "Gentle Hills",
				"amplitude": gentle_hills_amplitude,
				"frequencia": gentle_hills_frequencia,
				"octaves": gentle_hills_octaves,
				"persistencia": gentle_hills_persistencia,
				"lacunaridade": gentle_hills_lacunaridade
			}
		"Rolling Hills":
			return {
				"tipo": "Rolling Hills",
				"amplitude": rolling_hills_amplitude,
				"frequencia": rolling_hills_frequencia,
				"octaves": rolling_hills_octaves,
				"persistencia": rolling_hills_persistencia,
				"lacunaridade": rolling_hills_lacunaridade
			}
		"Valleys":
			return {
				"tipo": "Valleys",
				"amplitude": valleys_amplitude,
				"frequencia": valleys_frequencia,
				"octaves": valleys_octaves,
				"persistencia": valleys_persistencia,
				"lacunaridade": valleys_lacunaridade
			}
		"Custom":
			return {
				"tipo": "Custom",
				"amplitude": custom_amplitude,
				"frequencia": custom_frequencia,
				"octaves": custom_octaves,
				"persistencia": custom_persistencia,
				"lacunaridade": custom_lacunaridade
			}
		_:
			return {
				"tipo": "Gentle Hills",
