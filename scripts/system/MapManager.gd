...
func _execute_load_map(map_scene_path: String, round_node: Node, actual_camera: Camera3D) -> bool:
	...
	if current_map.has_method("set_camera"):
...
func _make_resources_unique(node: Node):
	...
	var terrain3d: Terrain3D = node.get_node_or_null("Terrain3D")
	if terrain3d:
		if terrain3d.has_method("get_data") and node.has_method("set_data"):
