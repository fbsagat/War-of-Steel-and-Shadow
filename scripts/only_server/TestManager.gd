...
func create_test_round(nome_sala_: String = "Sala de Teste"):
	...
	# Apagar futuramente, não é mais útil, agora fica por conta do _handle_start_round
	## Cria rodada
	...
	#round_node.name = "Round_%d_%d" % [room_data["id"], round_data["id"]]
	#
...
#
	...
	#
		...
		#return
	#
	#_log_debug("  ✓ Rodada criada: ID %d" % round_data["id"])
		#
	#await get_tree().process_frame
	#
	...
	#
		...
		#
	## Gera spawn points
	#var players_qtd = round_registry.get_total_players(round_data["id"])
	...
	#for room_player in match_data["players"]:
		#client_registry.set_player_state(room_player["uuid_base"], client_registry.ClientState.IN_GAME)
...
#
	...
	#_log_debug("  Rodada: %d" % round_data["id"])
	#_log_debug("==============================================")
...
#func _spawn_player_on_server(player_data: Dictionary, spawn_data: Dictionary, round_id: int, players_node):
	## VALIDAÇÕES INICIAIS
	#if not player_data.has("uuid_base") or not player_data.has("name"):
...
#func on_spawn_requested(_character) -> void:
	#var uuid_base = client_registry.get_uuid_by_peer_id(_character.session_id)
