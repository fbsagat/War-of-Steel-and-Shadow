...
@export var sync_rate: float = 0.03 # 33 updates/segundo
...
@export var anim_sync_rate: float = 0.1  # 10 updates/segundo
...
func _on_hitbox_body_entered(body: Node, hitbox_area: Area3D) -> void:
	...
	if body.is_in_group("enemy") or body.is_in_group("remote_player") and (is_attacking or is_block_attacking):
		...
		if server_manager and server_manager.has_method("attack_validation"):
			server_manager.attack_validation(group, peer_id, actual_weapon.name, body.peer_id)
...
func _log_debug(message: String):
	...
	print("%s[PlayerNode][S_ID: %d][Nome: %s]: %s" % [prefix, peer_id, player_name, message])
